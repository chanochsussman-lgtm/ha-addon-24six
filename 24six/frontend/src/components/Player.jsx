...
export default function Player({ onSpeakerClick }) {
  ...
  return (
    ...
    {/* ── Mini player bar ───────────────────────────────────────────── */}
    <div
      className="fixed bottom-0 glass border-t flex items-center gap-3 px-4 z-50"
      style={{ height: 'var(--player-height)', borderColor: 'var(--border)', left: 'var(--nav-width)', right: 0 }}
    >
      ...
      <div className="flex-1 flex flex-col items-center gap-1" style={{ maxWidth: 500 }}>
        {/* Control row */}
        <div className="flex items-center gap-2">
          ...
          {/* 10s rewind */}
          <Btn onClick={rewind10} title="-10s" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-secondary)' }}>
            <Rewind10Icon size={18} />
          </Btn>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <Btn
          active={false}
          onClick={() => currentSong && navigate(`/lyrics/${currentSong.id}`)}
          title="View Lyrics"
        ><LyricsIcon size={16} /></Btn>
        ...
        <div className="relative" onClick={e => e.stopPropagation()}>
          ...
          >{speed}×</button>
          {showSpeed && (
            <div className="absolute bottom-10 right-0 rounded-xl py-2 w-28 z-20 shadow-2xl"
        ...
        <div className="relative" onClick={e => e.stopPropagation()}>
          ...
          {showQuality && (
            <div className="absolute bottom-10 right-0 rounded-xl py-2 w-24 z-20 shadow-2xl"
              style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
              {QUALITIES.map(q => (
                <button key={q.v}
                  className="w-full text-left px-4 py-1.5 text-sm transition-colors hover:bg-surface"
        ...
        <input type="range" className="w-20" min={0} max={1} step={0.01} value={volume}
      ...
      <div className="relative z-10 flex flex-col h-full max-w-lg mx-auto w-full px-6 pt-4 pb-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex-1 min-w-0 mr-4">
            <p className="font-display text-2xl font-semibold truncate-2 leading-tight mb-1" style={{ color: '#fff' }}>
              {title}
            </p>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button className="p-2" style={{ color: 'rgba(255,255,255,0.8)' }} onClick={prev}>
            <PrevIcon size={28} />
          </button>
          ...
          <button className="p-2" style={{ color: 'rgba(255,255,255,0.8)' }} onClick={next}>
            <NextIcon size={28} />
          </button>

          <button
            className="p-2"
            style={{ color: repeat !== 'none' ? 'var(--accent)' : 'rgba(255,255,255,0.5)' }}
        ...
        <div className="flex items-center justify-between">
          {/* ±10s */}
          <button className="p-2" style={{ color: 'rgba(255,255,255,0.5)' }} onClick={rewind10} title="-10s">
          ...
          <button
            ...
            onClick={() => { navigate(`/lyrics/${currentSong?.id}`); onClose() }}
            title="Lyrics"
          ><LyricsIcon size={20} /></button>
          ...
          <div className="relative" onClick={e => e.stopPropagation()}>
            <button
              ...
              style={{
                color: speed !== 1 ? 'var(--accent)' : 'rgba(255,255,255,0.5)',
                background: showSpeed ? 'rgba(255,255,255,0.1)' : 'transparent',
              }}
            ...
            {showSpeed && (
              <div className="absolute bottom-10 left-1/2 -translate-x-1/2 rounded-xl py-2 w-28 z-20 shadow-2xl"
                style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
                {SPEEDS.map(s => (
                  <button key={s} className="w-full text-left px-4 py-1.5 text-sm hover:bg-surface transition-colors"
                    style={{ color: speed === s ? 'var(--accent)' : 'var(--text)', fontWeight: speed === s ? 600 : 400 }}
                    onClick={() => { setSpeed(s); setShowSpeed(false) }}
                  >{s}×</button>
                ))}
          ...
          <div className="relative" onClick={e => e.stopPropagation()}>
            <button
              ...
              style={{ color: sleepTimer ? 'var(--accent)' : 'rgba(255,255,255,0.5)' }}
              onClick={() => setShowSleep(s => !s)}
              title="Sleep timer"
            >
              {sleepTimer
                ? <span className="text-xs font-semibold tabular-nums">{formatSleep(sleepRemaining)}</span>
                : <SleepIcon size={20} />}
            </button>
            {showSleep && (
              <div className="absolute bottom-10 right-0 rounded-xl py-2 w-36 z-20 shadow-2xl"
                ...
                <p className="px-4 py-1 text-xs font-semibold" style={{ color: 'var(--muted)' }}>SLEEP TIMER</p>
                {sleepTimer && (
                  <button className="w-full text-left px-4 py-1.5 text-sm hover:bg-surface"
                    style={{ color: '#e57373' }}
                    onClick={() => { clearSleepTimer(); setShowSleep(false) }}
                  >Cancel ({formatSleep(sleepRemaining)})</button>
                )}
                {SLEEP_PRESETS.map(m => (
                  <button key={m} className="w-full text-left px-4 py-1.5 text-sm hover:bg-surface"
                    style={{ color: 'var(--text)' }}
                    onClick={() => { setSleepTimer(m); setShowSleep(false) }}
                  >{m >= 60 ? `${m / 60}h` : `${m} min`}</button>
                ))}
          ...
          <button
            className="p-2"
            style={{ color: entity_id ? 'var(--accent)' : 'rgba(255,255,255,0.5)' }}
            onClick={onSpeakerClick} title="Speaker"
          ><SpeakerIcon size={20} /></button>

          {/* +10s */}
          <button className="p-2" style={{ color: 'rgba(255,255,255,0.5)' }} onClick={forward10} title="+10s">
        ...
        <div className="flex items-center gap-3 mt-4">
          <button style={{ color: 'rgba(255,255,255,0.4)' }} onClick={() => setVolume(0)}>
            <MuteIcon size={16} />
          </button>
          <input
            type="range" className="flex-1" min={0} max={1} step={0.01} value={volume}
            onChange={e => setVolume(Number(e.target.value))}
          />
          <button style={{ color: 'rgba(255,255,255,0.4)' }} onClick={() => setVolume(1)}>
            <VolumeIcon size={16} />
          </button>
        </div>
      </div>
...
function Btn({ children, active, onClick, title, style }) {
  return (
    <button onClick={onClick} title={title}
      className="p-1.5 rounded transition-colors hover:text-text"
